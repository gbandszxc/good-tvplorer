...
function Save-Icon([System.Drawing.Image]$Image, [string]$Destination, [int]$Size) {
...
try {
    ...
    foreach ($density in $densities.GetEnumerator()) {
        ...
        Save-Icon $image (Join-Path $drawable "ic_launcher_foreground.png") $density.Value[1]
    }
} finally {
    $image.Dispose()
}

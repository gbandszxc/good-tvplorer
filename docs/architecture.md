...
SMB 连接信息 `SmbConnectionInfo` 保存在 DataStore Preferences 中（当前为 MVP 明文存储，后续改为加密）。
...
1. **安全**：SMB 密码从明文 DataStore 改为 `EncryptedSharedPreferences` 或 Android Keystore。
